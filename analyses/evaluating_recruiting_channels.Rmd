...
What I'm doing, what this is for (assumption -- already have R, possibly R Studio installed)
...
```{r}
library(psych)
# describe attrition by hiresource, and put it into a matrix
describeBy(mydata$Attrition, mydata$HireSource, mat = TRUE)  
```

I like this package because the information is output close together, so it is easy to compare. I don't always care about range or skew, but if I did, it's all there. Here I'm most curious in the mean. Is the average attrition the same for each recruiting channel, or are there differences?

<span style="color:blue">what graphs should I be using?</span>
Visually comparing the means, 
...
<span style="color:blue">t-test, regression</span>
